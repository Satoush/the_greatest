...
class Character():
    ...
    # Splits the spritesheets into individual images
    def get_image(self, frame_of_image, scale,colour):
        image = pygame.Surface((self.width,self.height)).convert_alpha()
        image.blit(self.sprite_sheets[self.current_state - 1],(0,0), (0, (frame_of_image * self.height),
        self.width, self.height))
        image = pygame.transform.scale(image,(self.width * scale, self.height * scale))
        image.set_colorkey(colour) # makes the background of the character rectangle transparent

        return image

    # Iterates through the animation steps and appends the frames to the master list
    def draw_character(self):
        for animation in self.animation_steps:
            temp_list = []
            self.current_state += 1 # this makes sure that the spritesheet moves on to the next image
            for i in range(animation):
                temp_list.append(self.get_image(self.step_counter,  3, self.colour))
                self.step_counter += 1
            self.step_counter = 0
    ...
    def animate_character(self,screen,colour):
        ...
        # would flip the image if the angle was less then 0
        if angle <= 0:
            ...
            new_image =  self.sprite[self.current_sprite_sheet][self.frame]
    ...
    # Creates the bullet object and fires it
    def fire(self):
        mx, my = pygame.mouse.get_pos()
    ...
    def update(self,screen,colour):
        self.animate_character(screen, colour)
        self.player_input()
        self.boundaries()
        #self.draw_character()