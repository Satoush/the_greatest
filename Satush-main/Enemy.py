...
class enemy():
    ...
    def get_image(self, the_current_frame, scale, colour, stage):
        ...
        image.blit(self.Image_path, (0, 0), ((the_current_frame * self.width),(stage * self.height),
        self.width, self.height)) # third argument specifies what part of the image to display(
    ...
    def animate_character(self,screen):
        ...
        if current_time - self.last_update >= self.cooldown:
            ...
            # makes sure the list doesnt go out of range
            if self.frame >= len(self.sprite[self.current_sprite_sheet]) - 1:
               self.frame = 0
    ...
    def move_to_player(self, PlayerX, PlayerY, screen):
    ...
    def update(self, PlayerX, PlayerY, screen):
        self.move_to_player(PlayerX, PlayerY,screen)
